...
module.exports = {
    register: async function(req, res) {
        try {
            ...
            if (!u) {
                ...
                res.status(403).json({message: 'Email alread registered', error: {}});
            }
        } catch (error) {
    ...
    login: function(req, res){
        ...
        UserModel.findOne({email: req.body.email}).lean().exec(function(err, user){
            ...
            return res.status(404).json({
                message: 'Wrong e-mail or passwrod'
            });
        });
    },
    check_token: function(req, res, next) {
        ...
        jwt.verify(token, consts.keyJwt, (err, decoded) => {
            if (err || !decoded) {
                return res.status(401).json({messge: 'Wrong token. Authentication error'});
            }
            next();
        });
    },
